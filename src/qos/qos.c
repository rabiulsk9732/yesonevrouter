...
int qos_tb_conform(struct token_bucket *tb, uint32_t pkt_len)
{
    ...
    /* Refill tokens */
    uint64_t new_tokens = (tb->rate * elapsed) / 1000000000ULL;
...
qos_action_t qos_meter_packet(struct qos_meter *meter, uint32_t pkt_len)
{
    ...
    uint64_t c_refill = (meter->cir * elapsed) / 1000000000ULL;
    ...
    uint64_t p_refill = (meter->pir * elapsed) / 1000000000ULL;
