...
/* Hash password using SHA-256 (OpenSSL 3.0 EVP API) */
static void hash_password(const char *password, char *hash_out)
{
    unsigned char hash[32];  /* SHA-256 produces 32 bytes */
    EVP_MD_CTX *mdctx;
    const EVP_MD *md;
    EVP_MD *md_fetched = NULL;  /* Track if we fetched (need to free) */
    unsigned int hash_len;

    /* Try OpenSSL 3.0 EVP_MD_fetch first */
    md_fetched = EVP_MD_fetch(NULL, "SHA256", NULL);
    if (md_fetched) {
        md = md_fetched;
    } else {
        /* Fallback to legacy EVP_get_digestbyname for older OpenSSL */
    ...
    EVP_MD_CTX_free(mdctx);
    /* Only free if we used EVP_MD_fetch (OpenSSL 3.0) */
    if (md_fetched) {
        EVP_MD_free(md_fetched);
    }
