                                      ...
                                      struct interface *egress_iface)
{
    ...
    uint16_t frag_offset = offset / 8;  /* Offset is in 8-byte units */
    uint16_t flags = more_fragments ? RTE_IPV4_HDR_MF_FLAG : 0;
    ip->fragment_offset = rte_cpu_to_be_16((flags << 13) | frag_offset);
    ...
    /* Copy fragment data */
    memcpy(fragment->data + sizeof(struct rte_ether_hdr) + ip_hdr_len,
           data, data_len);
                       ...
                       struct interface *egress_iface)
{
    ...
    uint8_t *payload_data = pkt->data + sizeof(struct rte_ether_hdr) + ip_hdr_len;
    uint16_t offset = 0;
