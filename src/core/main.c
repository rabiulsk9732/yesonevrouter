...
int main(int argc, char *argv[])
{
    ...
    /* VPP-STYLE: Initialize worker handoff rings for lockless operation */
#ifdef HAVE_DPDK
    extern int nat_worker_handoff_init(uint32_t num_workers);
    if (nat_worker_handoff_init(nat_workers) != 0) {
        YLOG_WARNING("Failed to init NAT handoff rings - using fallback path");
    } else {
        YLOG_INFO("NAT worker handoff rings initialized for %d workers", nat_workers);
    }
#endif
